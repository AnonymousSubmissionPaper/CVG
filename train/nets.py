...
class batch_gnn(torch.nn.Module):
    def __init__(self, in_channels, hidden_channels, encoder_mu=None,encoder_logstd=None, output_type='both'):
        super(batch_gnn, self).__init__()
        self.output_type=output_type
        self.hidden_channels = hidden_channels
        if encoder_mu==None:
            self.encoder_mu=Encoder(in_channels, hidden_channels,encoder_type='GCN')
        else:
            self.encoder_mu=encoder_mu
        if encoder_logstd==None:
            self.encoder_logstd=Encoder(in_channels, hidden_channels,encoder_type='GCN')
        else:
            self.encoder_logstd=encoder_logstd
    ...
    def reset_parameters(self):
        reset(self.encoder_mu)
        reset(self.encoder_logstd)
        reset(self.pool)


    def forward(self, x, edge_index, edge_attr, batch, index=None, return_hidden=True):
        r""" Return node and subgraph representations of each node before and after being shuffled """

        hidden = (self.encoder_mu(x, edge_index, edge_attr, batch),
                  self.encoder_logstd(x, edge_index, edge_attr, batch))
...
class VGCN_Encoder(torch.nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.conv2= GCNConv(in_channels, 2 * out_channels, cached=False)
    ...
    def forward(self, x, edge_index, edge_attr=None, return_hidden=True):
        ...
        x2= self.conv2(x, edge_index).relu()
        return self.conv_mu(x1, edge_index), self.conv_logstd(x2, edge_index)
...
class V_Encoder(torch.nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        #self.conv1 = GCNConv(in_channels, 2 * out_channels, cached=False) # cached only for transductive learning
        #self.conv2= GCNConv(in_channels, 2 * out_channels, cached=False)
        self.conv_mu = GCN(in_channels=in_channels, hidden_channels=2 * out_channels, out_channels=out_channels,
                               num_layers=3, dropout=0.2)
        self.conv_logstd = GCN(in_channels=in_channels, hidden_channels=2 * out_channels, out_channels=out_channels,
                               num_layers=3, dropout=0.2)
