...
class LEGNN(torch.nn.Module):
    def __init__(self, in_channels, hid_channels=64, num_unit=2):
        super().__init__()
        ...
        for i in range(num_unit):
            ...
            self.convs.append(conv)
            self.relus.append(ReLU())
            self.batch_norms.append(torch.nn.BatchNorm1d(hid_channels))
    ...
    def get_node_reps(self, x, edge_index, edge_attr, batch):
        x = self.node_emb(x)
        for i, conv, ReLU, batchnorm in \
                zip(list(range(len(self.convs))),self.convs, self.relus, self.batch_norms):
            x = conv(x=x, edge_index=edge_index, edge_weight=edge_attr)
            x=batchnorm(x)
